    ...
    <div className="game-active">
      ...
      <div className="game-active-box">
        ...
        <div className="game-active-tabs">
          {activeList.map(item => <div onClick={() => setActiveUrl(item.name)} className={`game-active-tab ${item.name === activeUrl ? 'game-active-tab-active' : ''}`} key={item.name}>{item.name}</div>)}
        </div>
      </div>
    </div>
  );
